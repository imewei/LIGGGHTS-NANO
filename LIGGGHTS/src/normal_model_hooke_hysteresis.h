...
namespace ContactModels
{
  ...
  public:
    ...
    inline void surfacesIntersect(SurfacesIntersectData & sidata, ForceData & i_forces, ForceData & j_forces)
    {
      ...
      bool viscous_val = this->viscous;
      double ** Yeff_val = this->Yeff;
      double charVel_val = this->charVel;
      bool tangential_damping_val = this->tangential_damping;
      ...
      if (viscous_val)  {
        double ** coeffMu_val = this->coeffMu;
        double ** coeffRestMax_val = this->coeffRestMax;
        double ** coeffStc_val = this->coeffStc;
        // Stokes Number from MW Schmeeckle (2001)
        const double stokes=sidata.meff*sidata.vn/(6.0*M_PI*coeffMu_val[itype][jtype]*reff*reff);
        // Empirical from Legendre (2006)
        coeffRestLogChosen=log(coeffRestMax_val[itype][jtype])+coeffStc_val[itype][jtype]/stokes;
      } else {
        double ** coeffRestLog_val = this->coeffRestLog;
        coeffRestLogChosen=coeffRestLog_val[itype][jtype];
      }

      const double sqrtval = sqrt(reff);
      double kn = 16./15.*sqrtval*(Yeff_val[itype][jtype])*pow(15.*meff*charVel_val*charVel_val/(16.*sqrtval*Yeff_val[itype][jtype]),0.2);
      double kt = kn;
      const double gamman = sqrt(4.*meff*kn/(1.+(M_PI/coeffRestLogChosen)*(M_PI/coeffRestLogChosen)));
      const double gammat = tangential_damping_val ? gamman : 0.0;
